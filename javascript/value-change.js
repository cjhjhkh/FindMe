...
window.addEventListener('load', () => {
    ...
    axios({
        ...
        alValue.forEach((item, index) => {
            // 添加已选价值观
            selValues.push(result.data.data[0].valuesList[index]);
            item.innerHTML = result.data.data[0].valuesList[index];
        })

        //获取除原有价值观以外价值观
        let swap = allVal.filter(item => !result.data.data[0].valuesList.includes(item));
    ...
    nextBtn.addEventListener('click', () => {
        ...
        //跳转
        location.href = './value-second-play.html';
    })
